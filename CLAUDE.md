...
- `src/restructure.ts` — **restructure-into-new-note engine.** Creates a NEW note from the current note + a configured **template ↔ main-folder pair** (`settings.restructureTemplates`). `loadRestructurePairs()` resolves each pair's template (frontmatter + full body, no LLM); `rankRestructurePairs()` LLM-ranks pairs for a note, leaning on its frontmatter (mirrors `rankTemplates`); `suggestSubfolder()` picks the best existing subfolder under the pair's target folder or proposes a new one; `restructureNoteBody()` reflows the source into the template's headings, filling each section per its **per-section guidance line** (summary/list/verbatim/etc.) using source-only content and stripping the guidance; `factCheckNoteBody()` is a second LLM pass that validates every number/key fact against the source, correcting and re-running (capped) until clean. Frontmatter is filled by reusing `fillTemplateFrontmatter()`/`applyTemplateFrontmatter()`.
...
- `src/settings-tab.ts` — settings UI panel (LLM gateway config, model dropdown, "Check" probe, reference groups, template folder, restructure-template pairs).
- `src/view.ts` — sidebar drop-zone UI and per-file/per-note actions ("Add summary", "Suggest tags", "Add references", "Fix to template", "Create restructured note"). Orchestrates the reference flow: extract metadata → `fillMetadataWithLLM` → `summarizeContent` → `matchCandidatesWithLLM` → open `ReferenceModal`; the template flow: `findTemplates` → `rankTemplates` → open `TemplateModal` (→ `fillTemplateFrontmatter` → `applyTemplateFrontmatter`); and the restructure flow: `loadRestructurePairs` → `rankRestructurePairs` → open `RestructureModal` (→ `suggestSubfolder` → `restructureNoteBody` → `factCheckNoteBody` → create note).
...
11. `rankRestructurePairs()` — pick the best template↔folder pair for the restructure workflow (`restructure.ts`)
12. `suggestSubfolder()` — pick/propose the target subfolder for a restructured note (`restructure.ts`)
13. `restructureNoteBody()` — reflow a note into a template's headings per per-section guidance (`restructure.ts`)
14. `factCheckNoteBody()` — verify/correct numbers & key facts in a restructured note against the source (`restructure.ts`)
