...
// A resolved restructure pair: its config, the template note it points at
...
export interface RestructurePair {
	config: RestructureTemplatePair;
	template: TemplateNote;
	body: string;
}

const RESTRUCTURE_RANK_TIMEOUT_MS = 60_000;
// Restructuring and fact-checking can be slow on reasoning models, mirror the
// long timeout used by summarizeContent.
const RESTRUCTURE_BODY_TIMEOUT_MS = 180_000;
...
// Resolve each configured pair into a RestructurePair, reading the template's
// frontmatter (from the metadata cache) and full body. Pairs whose template
// file is missing or whose target folder is blank are skipped.
export async function loadRestructurePairs(
	app: App,
	pairs: RestructureTemplatePair[],
): Promise<RestructurePair[]> {
	const out: RestructurePair[] = [];
...
// the call fails.
export async function rankRestructurePairs(
	noteTitle: string,
	noteFrontmatter: Record<string, unknown>,
	noteBody: string,
	pairs: RestructurePair[],
	gateway: LlmGateway | null,
	persist?: () => Promise<void>,
): Promise<{ ranked: RestructurePair[]; usedLlm: boolean }> {
	...
	const system =
		'You choose which restructure template a note belongs to. ' +
	...
	const result = await callChat(
		gateway,
		{
			...
			maxTokens: 200,
			temperature: 0,
			timeoutMs: RESTRUCTURE_RANK_TIMEOUT_MS,
		},
		persist,
	);
...
// Restructure the source note into the pair's template: keep the template's
...
export async function restructureNoteBody(
	pair: RestructurePair,
	noteTitle: string,
	noteFrontmatter: Record<string, unknown>,
	noteBody: string,
	gateway: LlmGateway | null,
	...
	const fmLines = fmLinesOf(noteFrontmatter);
	const system =
		'You restructure an existing note into a target template. ' +
	...
	const result = await callChat(
		gateway,
		{
			...
			maxTokens: 4000,
			temperature: 0,
			timeoutMs: RESTRUCTURE_BODY_TIMEOUT_MS,
		},
		persist,
	);
	if (!result.ok) return result;
...
// Verify the restructured body against the source: every number, date, name and
...
export async function factCheckNoteBody(
	...
	const system =
		'You verify a restructured note against its source. ' +
		'Check every number, date, name, quantity and key fact in the restructured note: each must be directly supported by the source. ' +
